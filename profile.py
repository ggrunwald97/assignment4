...
for i in range(5):
  ...
  node.addService(pg.Execute(shell="sh", command="sudo su AN899916 -c 'cp /local/repository/source/* /users/AN899916'"))
  ...
  if i > 2: # Compute Nodes
    ...
    node.addService(pg.Execute(shell="sh", command="sudo -H -u AN899916 bash -c '/local/repository/scripts/mpi_path_setup.sh'"))
